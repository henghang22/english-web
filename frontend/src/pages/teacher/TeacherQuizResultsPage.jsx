...
import { 
  ...
  ArrowDown, Eye, X, HelpCircle, Info, ShieldCheck, CheckCircle2, BookOpen, Award
} from 'lucide-react';
...
// Chỉ cần đổi endpoint từ admin sang teacher là dùng được gần như toàn bộ giao diện Admin
  ...
  const fetchResults = async () => {
    try {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/teacher/quiz-results`,
        {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 pb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Theo dõi điểm số và tiến độ của học viên trong các khóa học của bạn
          </p>
        </div>
        ...
        {filteredResults.length === 0 && (
          <div className="py-16 text-center">
            ...
            <p className="text-xs text-slate-400 mt-1">
              Dữ liệu sẽ xuất hiện khi học viên hoàn thành bài thi
            </p>
          </div>
        )}
      </div>
