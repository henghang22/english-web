...
import {
  ...
  HelpCircle, Clock, Pencil, Loader2, AlertCircle, Sparkles, Info
} from 'lucide-react';
...
const AdminQuizPage = () => {
  ...
  const fetchQuizData = async () => {
    try {
      ...
      const endpoint = lessonId
        ? `${import.meta.env.VITE_API_URL}/admin/lessons/${lessonId}/quiz`
        : `${import.meta.env.VITE_API_URL}/admin/courses/${courseId}/quiz`;

      const quizRes = await axios.get(endpoint, { headers });
      const quizData = quizRes.data.data;
      setQuiz(quizData);
      if (quizData) {
        setQuizFormData({ title: quizData.title, time_limit: quizData.time_limit });
      }
    } catch (error) {
  ...
  const handleUpdateQuiz = async () => {
    ...
    try {
      const token = JSON.parse(localStorage.getItem('auth-storage'))?.state?.token;
      await axios.put(
        `${import.meta.env.VITE_API_URL}/admin/quizzes/${quiz.id}`,
        quizFormData,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!answers.some(a => a.is_correct)) return toast.error('Vui lòng chọn ít nhất một đáp án đúng');
    ...
    try {
      ...
      if (editingQuestionId) {
        await axios.put(
          `${import.meta.env.VITE_API_URL}/admin/questions/${editingQuestionId}`,
          payload,
          { headers }
        );
        ...
        await axios.post(
          `${import.meta.env.VITE_API_URL}/admin/quizzes/${quiz.id}/questions`,
          payload,
          { headers }
        );
  ...
  const handleDelete = async () => {
    try {
      const token = JSON.parse(localStorage.getItem('auth-storage'))?.state?.token;
      await axios.delete(`${import.meta.env.VITE_API_URL}/admin/questions/${deletingId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
